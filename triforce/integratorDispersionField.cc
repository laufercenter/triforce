...
IntegratorDispersionField::IntegratorDispersionField(){
	
}


IntegratorDispersionField::IntegratorDispersionField(vector<vector<Interpolator*> > data){
	this->data=data;
...
void IntegratorDispersionField::clearForces(){
	purgeForces();
...
void IntegratorDispersionField::pushForces(){
...
void IntegratorDispersionField::purgeForces(){
	forcesDelayed.clear();
}
...
void IntegratorDispersionField::addForce(int i, Vector force){
	ForceElement fe;
	if(i>=0){
...
bool IntegratorDispersionField::isWithinNumericalLimits(float x, float l){
	if(abs(x-l)<=THRESHOLD_NUMERICAL) return true;
	else return false;
}


Vector  IntegratorDispersionField::recoverCircularInterface(Vector p, float psi_b, float lambda_b, float PHI_b0){
	Vector c(2);
...
Vector  IntegratorDispersionField::recoverCircularInterface(float psi_a, float lambda_a, float PHI_a1, float psi_b, float lambda_b, float PHI_b0){
	Vector c(2);
...
Vector  IntegratorDispersionField::intersectionPoint(Vector c, float psi, float lambda, float PHI){
	Vector r(2);
	float d;
	d = norm(c,2);
...
float IntegratorDispersionField::integrateSASA(int l, SASASegmentList &sasa, float radius){
...
Vector IntegratorDispersionField::pwmult(Vector &a, Vector &b){
	Vector c(a.size());
...
Vector IntegratorDispersionField::lookUp(float PHI, float psi, float lambda, float kappa, Vector &surfaceparameter, CircularInterfaceForm form){
	Vector res(4);
	float aPHI, apsi, alambda;
	int i;
	Vector v(5);
	Vector x(dimSingularVectors);
	aPHI=abs(PHI);
	apsi = max(0.0f,psi);
	alambda= max(0.0f,lambda);
	akappa= max(0.0f,kappa); //@TODO this has to be checked
	...
	res(0)=1;
	for(unsigned j=0; j<=dimSurfaceParameter; ++j){
		v(4)=surfaceparameter(j);
		
		x = pwmult(x,  -(data[i][j]->interpolate(v)));
		
		/*
		for(i=0;i<4;++i){
			//printf("start lookup %d (%f %f %f)\n",i,PHI,psi,lambda);
			res(i) = -(data[i][j]->interpolate(v));
			//printf("end lookup %d\n",i);
		}
	...
	res(0)=0;
	for(unsigned j=0; j<=dimSingularVectors; ++j){
		res(0)=res(0)+x(j);
	}
...
float IntegratorDispersionField::PHI2phi(float PHI, float psi, float lambda){
...
float IntegratorDispersionField::PHI2phi2(Vector integrationOrigin, float PHI, float psi, float lambda){
	fmat33 T;
	fmat33 r0, r1;
	Vector n(3);
	Vector n0(3);
...
bool IntegratorDispersionField::isInPositiveEpsilonRange(float v, float eps){
	if(eps-(v+THRESHOLD_NUMERICAL) <= 0) return true;
	else return false;
}


float IntegratorDispersionField::V2phi(Vector &integrationOrigin, Vector cv, Vector &v){
	Vector up(3);
	Vector n_origin(3);
	Vector n_v(3);
	Vector n_cv(3);
...
fmat33 IntegratorDispersionField::rotz(float theta){
	fmat33 m;
	m(0,0) = cos(theta);
	m(0,1) = -sin(theta);
	m(0,2) = 0;
...
float IntegratorDispersionField::logisticSmoother(float x){
	return 1.0/(1.0+exp(-((x*2*LOGISTIC_SMOOTHER_PARAMETER)-LOGISTIC_SMOOTHER_PARAMETER)));
}



float IntegratorDispersionField::sech(float x){
...
float IntegratorDispersionField::dlogisticSmoother(float x){
...
Vector IntegratorDispersionField::areaSmoother(Vector &x, float area, float radius){
	return x*area/(2*M_PI*radius*radius);
}



Integral IntegratorDispersionField::integrateTriangle(int l, SASASegment &x, Vector integrationOrigin){
	Rotation psi;
	Rotation lambda;
	Rotation PHIij;
	Rotation PHIjk;
...
float IntegratorDispersionField::angle(Vector &a, Vector &b){
	return acos(norm_dot(a,b));
}

float IntegratorDispersionField::complAngle(Vector &a, Vector &b){
	return asin(norm_dot(a,b));
}

int IntegratorDispersionField::sgn(float d){
	if(d>=0) return 1;
	else return -1;
}
...
float IntegratorDispersionField::csc(float a){
	return 1.0/sin(a);
}
...
Benchmark IntegratorDispersionField::getBenchmark(){
	return benchmark;
}
